...
def run_retrosynthesis(smiles, database_name=None, template_file=None,
    ...
    # If target SMILES has no stereochemistry, strip bond stereo before matching
    # so that non-isomeric targets (e.g., C=CC) match isomeric templates (e.g., /C=C/).
    _has_stereo = any(c in smiles for c in ('/', '\\', '@'))
    if not _has_stereo:
        mol_stripped = Chem.MolFromSmiles(smiles)
        if mol_stripped:
            for bond in mol_stripped.GetBonds():
                bond.SetStereo(Chem.rdchem.BondStereo.STEREONONE)
            match_smiles = Chem.MolToSmiles(mol_stripped)
        else:
            match_smiles = smiles
    else:
        match_smiles = smiles

    p_mol_rdchiral = rdchiralReactants(match_smiles)
    p_mol = Chem.MolFromSmiles(match_smiles)
