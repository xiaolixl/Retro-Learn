  ...
  <div class="mol-container">{target_svg}</div>
    ...
    if mode == "single_step":
        ...
        for route in routes:
            ...
            cond = ", ".join(route.get("reaction_condition", []))
        ...
        # Multi-step — reverse the steps to show forward direction
        route = data["data"].get("recommended_route")
        if route:
            steps = route.get("steps", [])
            # Build forward chain: reactants of step N → ... → target
            # Collect all molecules in forward order
            chain_parts = []

            # Start with the final leaf reactants
            final_leaves = route.get("final_leaf_reactants", [])
            for li, leaf in enumerate(final_leaves):
                leaf_svg = smiles_to_svg_full(leaf["smiles"], 200, 160)
                chain_parts.append(_make_mol_box(leaf_svg, leaf["smiles"], "Starting material", stock=leaf.get("in_stock")))
                if li < len(final_leaves) - 1:
                    chain_parts.append('<span class="plus">+</span>')

            # For each step (in reverse), add: arrow → expanded molecule
            for step in reversed(steps):
                n = step["step_number"]
                expanded = step.get("expanded_smiles", "")
                cond = ", ".join(step.get("reaction_condition", []))
                score = step.get("step_score", 0)
                rxn_label = classify_reaction(step.get("reaction_template", ""), step.get("reaction_condition", []))
                rxn_cls = _rxn_type_class(rxn_label)

                exp_svg = smiles_to_svg_full(expanded, 200, 160)
                chain_parts.append(_make_arrow_section(cond, rxn_label, rxn_cls))
                chain_parts.append(_make_mol_box(exp_svg, expanded))

            # Show as a continuous chain:
            parts.append(f'<div class="route-card route-best">')
            parts.append(f'  <div class="route-header">')
            parts.append(f'    <span class="route-badge">Best Route</span>')
            parts.append(f'    <span class="route-score">Score: {route.get("route_score", 0):.4f} · {route.get("actual_steps", 0)} step(s)</span>')
            parts.append(f'  </div>')
