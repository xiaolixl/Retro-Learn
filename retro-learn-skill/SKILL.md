...
**After engine run**: Always supplement with **exactly 1 LLM-designed route** (regardless of engine success). The LLM route provides an alternative mechanistic perspective (e.g., known textbook routes, named reactions) that the template engine may miss.
...
**Direct beam search** (Scenario C): Run beam search directly with 5 steps (no incremental trial). Engine checks preferred reactant hit internally at each step.
